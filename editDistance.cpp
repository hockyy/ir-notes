...
string S, T;
int memo[5005][5005];
map <char, int> lastOccurenceS, lastOccurenceT;
    ...
    for(int i = 1;i <= n;i++){
      ...
      for(int j = 1;j <= m;j++){
        ...
        memo[i][j] = min(memo[i][j], memo[i - 1][j - 1] + kSubstitutionCost * (S[i] != T[j]));
        {
          ...
          int previousSameCharacterT = (lastOccurenceT.count(S[i]) ? lastOccurenceT[S[i]] : -1);
          if(previousSameCharacterS != -1 && previousSameCharacterT != -1){
